...
ifeq ($(TARGET_N64),0)
  ...
  ifeq ($(TARGET_IOS),1)
    ...
    # -fno-math-errno lets clang emit hardware instructions for sqrtf and
    # friends instead of libm calls that maintain errno, which nothing here
    # ever reads
    OPT_FLAGS := -O3 -flto -DNDEBUG -fno-math-errno
  endif
endif
...
# The assembler and linker script preprocessor need _LANGUAGE_ASSEMBLY when
# targeting the N64
ifeq ($(TARGET_N64),1)
  CPP_LANG_DEFINES := -D_LANGUAGE_ASSEMBLY
else
  CPP_LANG_DEFINES :=
endif
...
ifneq (,$(call find-command,clang))
  CPP      := clang
  CPPFLAGS := -E -P -x c -Wno-trigraphs $(CPP_LANG_DEFINES) $(DEF_INC_CFLAGS)
else
  CPP      := cpp
  CPPFLAGS := -P -Wno-trigraphs $(CPP_LANG_DEFINES) $(DEF_INC_CFLAGS)
endif
